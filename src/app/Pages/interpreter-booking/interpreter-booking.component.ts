...
export class InterpreterBookingComponent implements OnInit {
  ...
  changeUser_Type(user_type: string): void {
    ...
    if (user_type == "history") {
      ...
      this.IntepreterHistory = JSON.parse(localStorage.getItem("callhistory"));
  ...
  joinstream(id: any) {
    ...
    console.log(id);
    this.IntepreterStreams.forEach(stream => {
      if (stream.id == id) {
        this.historyPLaceholder.push(stream)
        localStorage.setItem("callhistory", JSON.stringify(this.historyPLaceholder));
      }
    });
    this.router.navigate(['/stream', id])
    // this.bookingsService.joinBooking(id).subscribe(
